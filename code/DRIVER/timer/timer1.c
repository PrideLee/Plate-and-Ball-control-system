...
void TIM1_Init(u16 auto_data,u16 fractional)
{
	TIM_TimeBaseInitTypeDef TIM_TimeBaseInitStructure;
	NVIC_InitTypeDef NVIC_InitStructure;
	
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_TIM1,ENABLE);      //使能TIM6时钟
  ...
  TIM_TimeBaseInitStructure.TIM_Period = auto_data; 	     //自动重装载值
	...
	TIM_ITConfig(TIM1,TIM_IT_Update,ENABLE); //允许定时器6更新中断
	TIM_Cmd(TIM1,DISABLE);                    //使能定时器6
